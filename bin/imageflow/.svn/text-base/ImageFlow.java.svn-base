...
public class ImageFlow extends JPanel {
	...
	private static final double ANIM_SCROLL_DELAY = 300;//450;
    private static final int CD_SIZE = 120; //148 68

    private int displayWidth = CD_SIZE;
    private int displayHeight = (int) (CD_SIZE * 2 / 1.12); //1.12
    ...
    public String getImageName(String avatarText){
		String retStr = "";
		
		String tempName = avatarText;
    ...
    public Dimension getPreferredSize() {
        return new Dimension(displayWidth * 5, (int) (displayHeight * 3) - 400);
    }
    ...
    private void drawAvatarName(Graphics2D g2) {
        ...
        TextLayout layout = new TextLayout(getImageName(avatarText), avatarFont, context);
        ...
        g2text.setColor(Color.white);//new Color(0, 0, 0, 170));
        layout.draw(g2text, 9, layout.getAscent() + 1);
        g2text.setColor(Color.black);
        layout.draw(g2text, 9, layout.getAscent());
        g2text.dispose();

        g2.drawImage(fx.createReflectedPicture(textImage,
                                               fx.createGradientMask((int) bulletWidth,
                                                                     (int) bulletHeight)),
                                            (int) x + 15, (int) y + 15, null);
        g2.setComposite(composite);
    }
    ...
    private void promoteAvatarToDrawable(List<DrawableAvatar> drawables,
        ...
        double spacing = offset * avatarSpacing*1.5;
        ...
        int avatarWidth = displayWidth*2;//avatar.getWidth(null);
        int avatarHeight = displayHeight*2;//avatar.getHeight(null);
...
//    }
    ...
    private class PicturesFinderThread implements Runnable {
        ...
        public void run() {
            ...
            //setAvatarIndex(0);
            //setAvatarIndex(avatarAmount / 2); lol
            startFader();
